...
function Thermostat:GetThermostatHeatSetpoint(Scale)
	Scale = Scale or self:GetTemperatureScale()
	return (Scale == Thermostat.FAHRENHEIT) and self._PersistData._SingleSetpointF or self._PersistData._SingleSetpointC, Scale
end
...
function Thermostat:ChangeCapability(CapName, CapVal)
	...
	if(SendChange) then
		...
		NOTIFY.CAPABILITIES_CHANGED(self._BindingID)
	end
end
